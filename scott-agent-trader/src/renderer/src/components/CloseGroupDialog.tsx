...
export default function CloseGroupDialog({
  ...
  const [stkQuotes, setStkQuotes] = useState<
  ...
  useEffect(() => {
    ...
    const fetchAll = async (): Promise<void> => {
      for (const [, c] of uniqueContracts) {
        try {
          const contracts = [
            { symbol: c.symbol, expiry: c.expiry, strike: c.strike, right: c.right }
          ]
  ...
  const previewRows = useMemo((): PreviewRow[] => {
    ...
    for (const pos of stockPositions) {
      ...
      const qty =
        stkQtyOverrides[overrideKey] !== undefined ? stkQtyOverrides[overrideKey] : pos.quantity
    ...
    for (const pos of optionPositions) {
      ...
      const qty =
        optQtyOverrides[overrideKey] !== undefined
                  ...
                  <div className="order-form" key={`stk-${sym}`} style={{ marginBottom: '20px' }}>
                    ...
                    >
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        ...
                        >
                          <span style={{ fontWeight: 600, fontSize: '13px', color: '#8b1a1a' }}>
                            賣出
                          </span>
                          <span
                            style={{
                        ...
                        <input
                          ...
                          value={stkPrices[sym] || ''}
                          onChange={(e) =>
                            setStkPrices((prev) => ({ ...prev, [sym]: e.target.value }))
                          }
                          placeholder="0.00"
                          step="0.01"
                    ...
                    >
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        ...
                        <input
                          ...
                          value={optPrices[key] || ''}
                          onChange={(e) =>
                            setOptPrices((prev) => ({ ...prev, [key]: e.target.value }))
                          }
                          className="input-field"
                          style={{ width: '90px' }}
                          step="0.01"
                        ...
                        {rows.map((row, idx) => {
                          ...
                          return (
                            <tr key={`${accountId}-${idx}`} style={{ height: '44px' }}>
                              ...
                              <td
                                style={{
                                  color: row.action === 'BUY' ? '#1a6b3a' : '#8b1a1a',
                                  fontWeight: 'bold'
                                }}
                              >
                                ...
                                {step === 'preview' ? (
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      if (isStk) {
                                        setStkQtyOverrides((prev) => ({
                                          ...prev,
                                          [overrideKey]: val
                                        }))
                                      } else {
                                        setOptQtyOverrides((prev) => ({
                                          ...prev,
                                          [overrideKey]: val
                                        }))
                                      }
