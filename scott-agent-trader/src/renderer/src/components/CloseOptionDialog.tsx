...
export default function CloseOptionDialog({
    ...
    const previews = useMemo((): CloseOptionPreview[] => {
        ...
        for (const [accountId, oMap] of accountOptionPositions) {
            ...
            for (const [key, posInfo] of oMap) {
                ...
                const price =
                    parseFloat(prices[key] || '') ||
                    (optQuotes[key]?.last || 0) ||
                    posInfo.avgCost
    ...
    const totalQty = previews.reduce(
        (s, p) => s + p.orders.reduce((ss, o) => ss + o.qty, 0),
        0
    )
            ...
            >
                ...
                <div className="stock-order-dialog-body">
                    ...
                    {uniqueContracts.map(([key, c]) => {
                        ...
                        return (
                            <div key={key} className="order-form" style={key !== uniqueContracts[0][0] ? { marginTop: '8px' } : undefined}>
                                <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
                                    <span style={{ fontWeight: 600, fontSize: '13px', color: actionColor, flex: '0 0 auto' }}>
                                        {action}
                                    </span>
                                    <span style={{ fontWeight: 600, fontSize: '13px', flex: '0 0 auto', minWidth: '140px' }}>{c.label}</span>
                            ...
                            <table className="allocation-table">
                                ...
                                <>
                                    {displayPreviews.map((p) => {
                                        ...
                                        const isLast =
                                            displayPreviews.indexOf(p) === displayPreviews.length - 1
                                        return (
                                            <tbody
                                                key={p.accountId}
                                                style={
                                                    isLast ? undefined : { borderBottom: '2px solid #e5e7eb' }
                                                }
                                            >
                                                {p.orders.map((order, idx) => {
                                                    const orderResult = orderResults.find(
                                                        (r) =>
                                                            r.account === p.accountId &&
                                                            r.symbol === order.label
                                                    )
                                                    ...
                                                    return (
                                                        <tr key={`${p.accountId}-${order.optKey}`} style={{ height: '44px' }}>
                                                            ...
                                                            <td
                                                                style={{
                                                                    color:
                                                                        order.action === 'BUY' ? '#1a6b3a' : '#8b1a1a',
                                                                    fontWeight: 'bold'
                                                                }}
                                                            >
                                                                {order.action === 'BUY' ? '買入' : '賣出'}
                                                            </td>
                                                            <td style={{ fontSize: '0.93em', paddingTop: '8px', paddingBottom: '8px' }}>{order.label}</td>
                                                            <td style={{ paddingTop: '8px', paddingBottom: '8px' }}>{prices[order.optKey] || '-'}</td>
                                                            <td>
                                                                {step === 'preview' ? (
                                                                    <input
                                                                        type="number"
