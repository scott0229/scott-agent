          ...
          <table className="allocation-table">
              ...
              <tr>
                <th style={{ width: '4%' }}>
                  <input
                    type="checkbox"
                    checked={
                      targetAccounts.length > 0 && checkedAccounts.size === targetAccounts.length
                    }
                    onChange={(e) => {
                      if (e.target.checked) {
            ...
            <tbody>
              {targetAccounts.map((acct) => {
                ...
                const stockAvgCost =
                  stockHolding > 0
                    ? stkPositions.reduce((sum, p) => sum + p.avgCost * p.quantity, 0) /
                      stockHolding
                    : 0
                return (
                  <tr key={acct.accountId}>
                    ...
                    <td
                      ...
                      onClick={() =>
                        setCheckedAccounts((prev) => {
                          const next = new Set(prev)
                          if (next.has(acct.accountId)) next.delete(acct.accountId)
                          else next.add(acct.accountId)
                          return next
                        })
                      }
                    >
                    ...
                    <td style={acct.totalCashValue < 0 ? { color: '#8b1a1a' } : undefined}>
                    ...
                    <td>
                      {(() => {
                        ...
                        const newGPV =
                          action === 'BUY'
                            ? acct.grossPositionValue + orderVal
                            : Math.max(0, acct.grossPositionValue - orderVal)
                    ...
                    {action === 'SELL' && (
                      <td>{stockAvgCost > 0 ? stockAvgCost.toFixed(2) : '-'}</td>
                    )}
                    {isChecked && !(action === 'SELL' && stockHolding === 0) ? (
                      <>
                        <td
                          style={{
                            color: action === 'BUY' ? '#1a6b3a' : '#8b1a1a',
                            fontWeight: 'bold'
                          }}
                        >
                          {action === 'BUY' ? '買入' : '賣出'}
            ...
            <table className="allocation-table">
              ...
              <tbody>
                {Object.entries(allocations).map(([accountId, qty]) => {
                  ...
                  const newGPV =
                    action === 'BUY'
                      ? currentGPV + orderValue
                      : Math.max(0, currentGPV - orderValue)
                  const postLeverage =
                    netLiq > 0 && newGPV > 0 ? (newGPV / netLiq).toFixed(2) : '無融資'
