...
export async function GET(req: NextRequest) {
    try {
        ...
        const { results: optionOpens } = await db.prepare(`
            SELECT id, owner_id, underlying as symbol, type as option_type, strike_price, quantity, premium as price, 'option' as asset_type, 'open' as action_type
            FROM OPTIONS
            WHERE date(datetime(open_date, 'unixepoch')) = ? AND owner_id IN (${userIds.join(',')})
        `).bind(dateStr).all();
        ...
        const { results: optionCloses } = await db.prepare(`
            SELECT id, owner_id, underlying as symbol, type as option_type, strike_price, quantity, final_profit as profit, 'option' as asset_type, 'close' as action_type
            FROM OPTIONS
